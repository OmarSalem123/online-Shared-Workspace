...
const QuillEditor:React.FC<QuillEditorProps> = ({ dirDetails, fileId, dirType}) => {
  ...
  const breadCrumbs = useMemo(() => {
    ...
    const folderSegments = segments[1];
    const folderDetails = workspaceDetails?.folders.find((folder) => folder.id === folderSegments);
    ...
    const fileDetails = folderDetails?.files.find((file) => file.id === fileSegments);
...
// Delete and restore folders and files
  ...
  const deleteBanner = async() => {
    ...
    if(dirType === 'file'){
        ...
        dispatch({
            type: 'UPDATE_FILE',
            payload: {file : { bannerUrl: ''}, folderId, workspaceId, fileId},
        });
        await supabase.storage.from('file-banners').remove([`banner-${fileId}`]);
        await updateFolder({ bannerUrl: ''}, fileId);
    }
  ...
  const iconOnChange = async(icon: string) => {
    ...
    if(dirType === "folder") {
        if(!workspaceId) return;
        dispatch({type: "UPDATE_FOLDER", payload: {folder: {iconId: icon}, workspaceId: workspaceId ,folderId: fileId},});
        await updateFolder({iconId: icon}, fileId);
    }
    if(dirType === "file") {
        if(!workspaceId || !folderId) return;
        dispatch({type: "UPDATE_FILE", payload: {file: {iconId: icon}, folderId, workspaceId, fileId},});
        ...
        {}
                ...
                <div className='flex flex-col-reverse sm:flex-row sm:justify-between justify-center sm:items-center sm:p-2 p-8'>
                    ...
                    <div className="flex items-center gap-4">
                                ...
                                <Tooltip>
                                    <TooltipTrigger>
                                        <Avatar className='-ml-3 bg-background border-2 flex items-center justify-center border-white h-8 w-8 rounded-full'>
                                            <AvatarImage className='rounded-full' src={collaborator.avatarUrl ? collaborator.avatarUrl : ''}/>
                        ...
                        <Badge variant={"secondary"} className='bg-orange-600 top-4 text-white right-4 z-50'>Saving...</Badge> 
                            ...
                            <Badge variant={"secondary"} className='bg-emerald-600 top-4 text-white z-50'>Saved</Badge>
                        )} 
                    </div>
                </div>
            </div>
            {details.bannerUrl && (
                <div className='relative w-full h-[200px]'>
                    ...
                    fill className='w-full md:h-48 h-20 object-cover' alt='BannerImage' />
                 </div>
            )}
            <div className='justify-center flex items-center flex-col mt-2 relative'>
                <div className='w-full self-center max-w-[800px] flex flex-col px-7 lg:my-8'>
                    ...
                    <div className='flex'>
                        <BannerUpload details={details} id={fileId} dirType={dirType} className="mt-2 text-sm text-muted-foreground p-2 hover:text-card-foreground transition-all rounded-md">{details.bannerUrl ? "Update Banner" : "Add Banner"}</BannerUpload>
                        {details.bannerUrl && (<Button disabled={deletingBanner} variant={'ghost'} className='gap-2 hover:bg-background flex items-center justify-center mt-2 text-sm text-muted-foreground w-36 p-2 rounded-md' onClick={deleteBanner}>
                            <XCircleIcon size={16}/>
